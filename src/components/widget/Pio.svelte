...
function getEffectivePixelRatio() {
	...
	const cap =
		save || (type && (type.includes("2g") || type.includes("slow-2g")))
			? 1.0
			: 1.25;
			...
			if (pioContainer && pioCanvas && !pioInitialized) {
				...
				try {
					if (pioInstance && typeof pioInstance.init === "function") {
						pioInstance.init();
						console.log("[PIO] pioInstance.init() called during initPio");
					}
				} catch (e) {}
...
onMount(() => {
							...
							for (const n of m.addedNodes) {
								try {
									if (
										...
										n.classList.contains("pio-close")
									) {
										n.remove();
									}
								} catch (e) {}
	...
	const handleShow = () => {
		...
		try {
			if (_overrideInterval) clearInterval(_overrideInterval);
		} catch (e) {}
		try {
			let tries = 0;
		...
		try {
			if (_classObserver) _classObserver.disconnect();
		} catch (e) {}
		try {
			if (typeof MutationObserver !== "undefined") {
				_classObserver = new MutationObserver((muts) => {
					for (const m of muts) {
						if (
							...
							pioContainer.classList.contains("hidden")
						) {
							try {
								pioContainer.classList.remove("hidden");
								pioContainer.classList.add("visible-manual-strong");
				...
				_classObserver.observe(pioContainer, {
					attributes: true,
		...
		try {
			if (typeof window !== "undefined" && window.__pendingPioShow) {
				try {
					delete window.__pendingPioShow;
				} catch (e) {}
		...
		try {
			if (typeof window !== "undefined")
				window.dispatchEvent(new CustomEvent("pio:shown"));
		} catch (e) {}
	};
	...
	try {
		...
		console.error("[PIO] force show failed", e);
	}
