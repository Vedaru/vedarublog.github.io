...
export const POST: APIRoute = async ({ request, locals }) => {
	...
	try {
		...
		if (needsChinese) {
			const suffix =
				"请用中文回答。除非用户明确要求其他语言，否则始终使用简体中文回复。";
	...
	if (cached && cached.expiresAt > now) {
		...
		const sess =
			clientSessionId || `s_${Math.random().toString(36).slice(2, 9)}`;
		return new Response(
			JSON.stringify({ ok: true, text: cached.text, sessionId: sess }),
			{
				status: 200,
	...
	try {
		...
		// default: return JSON
		return new Response(JSON.stringify({ ok: true, text: reply, sessionId }), {
			status: 200,
			headers: { "Content-Type": "application/json; charset=utf-8" },
		});
	} catch (err: any) {
